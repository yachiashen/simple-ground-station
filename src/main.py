...
# Define a default empty map content
EMPTY_MAP_CONTENT = "<html><body><p style=\\'text-align: center; margin-top: 20px;\\'>地圖將在此處顯示。</p></body></html>"

# Attempt to get map styles from visualizer, otherwise use a default list
...
# Check if visualizer has AVAILABLE_MAP_STYLES, otherwise use the default
...
# Static map styles for GIF generation (Cartopy based)
...
# DEFAULT_MAP_STYLE = MAP_STYLE_CHOICES[0] # This line is now effectively replaced by DEFAULT_STATIC_MAP_STYLE
DEFAULT_GIF_DURATION_MINUTES = 5
DEFAULT_GIF_FPS = 10

# --- Logger Setup ---
LOG_FILE_PATH = os.path.join(DATA_DIR, "app.log") # Use DATA_DIR

# 確保 data 目錄存在
if not os.path.exists(DATA_DIR): # Use DATA_DIR
    os.makedirs(DATA_DIR) # Use DATA_DIR

logger = logging.getLogger(__name__)

# Global TLE data and satellite object
tle_data = []
# app_state to store application state across Gradio interactions
app_state = {}
...
def format_current_position(pos_data, sat_name):
    ...
    # Use <br> for newlines in Markdown
    return (
...
def format_pass_predictions(pass_data, sat_name, observer_lat, observer_lon):
    ...
    table_data = []
    # Wrap column headers with nowrap span as well
    column_headers = [
...
def update_map_for_time_slider(slider_minutes_offset, current_app_state):
    ...
    try:
        ...
        # Add initial debug state to status_log
        ...
        track_ref_time_utc_obj = updated_app_state.get("track_reference_time_utc")  # This is now a Skyfield Time object
        ...
        # track_ref_time_utc_obj is already a Skyfield Time object, no need to parse from string
        track_reference_time_utc = track_ref_time_utc_obj

        # For Skyfield Time objects, use timedelta with appropriate conversion
        current_display_time_utc = track_reference_time_utc + timedelta(minutes=slider_minutes_offset)
        
        # More detailed logging for status_log
        ...
        if hasattr(satellite_obj, 'at'): 
            ...
            # current_display_time_utc is already a Skyfield Time object, use it directly
        ...
        folium_map_html_output, newly_computed_track_slider = visualizer.generate_interactive_map_folium(
            ...
            track_reference_time_utc=track_reference_time_utc, # This is the reference for the whole track
            display_time_utc=current_display_time_utc,       # This is for the current satellite marker & footprint
        ...
        status_log += "地圖已根據時間滑桿更新（固定縮放/中心更新）。<br>" # Original message
...
def ground_station_interface(
    ...
    # Initialize variables that will be returned, especially in error cases
    current_pos_str_val = ""
    passes_str_val = ""
    folium_map_html_output = EMPTY_MAP_CONTENT # Default for error cases
    # No download buttons needed
    slider_update = gr.update(visible=False, label="時間軸") # For both success and error slider updates
    ...
    try:
        ...
        ts = load.timescale() # Load timescale here
        app_state["ts"] = ts # Store in app_state if needed by other functions like slider update

        # --- Input Validation ---
        if not satellite_name_query:
            status_log += "錯誤：請輸入衛星名稱。<br>"
            logger.warning("ground_station_interface: Satellite name missing.")
            # Return current app_state if error occurs early
            # Ensure all return values match the output signature
        ...
        try:
            ...
            # Create Topos object for the observer
            ...
            app_state['current_observer_topos_obj'] = observer_topos_obj # Store the Topos object
        ...
        try:
            # Corrected: Call load_satellites_from_tle from satellite_tracker module
        ...
        except ValueError as e: # Catch TLE format errors specifically
        ...
        if not target_satellite:
            ...
            available_sats = [s.name for s in sats[:20]] # Show some available names
        ...
        app_state['current_observer_topos_obj'] = observer_topos_obj # Use the created Topos object
        
        tle_lines_content = None
        # Try to get TLE from satellite object first, then from file
        if hasattr(target_satellite, 'tle') and isinstance(target_satellite.tle, (list, tuple)) and len(target_satellite.tle) >= 2:
            # Assuming target_satellite.tle might be a list/tuple of [line1, line2]
            tle_lines_content = "\\n".join(target_satellite.tle)
        elif hasattr(target_satellite, 'tle_lines') and isinstance(target_satellite.tle_lines, str): # If .tle_lines is a string
        ...
        app_state['satellite_name'] = satellite_name_query # Use the input parameter
        ...
        prediction_duration_hours = 24.0  # Predict for next 24 hours
        ...
        if passes:
            # Create HTML table for pass predictions
            ...
            for i, pass_info in enumerate(passes[:10]):  # Show up to 10 passes in table
                passes_str_val += f"""
                    <tr>
        ...
        # Generate interactive map for the initial view
        folium_map_html_output, newly_computed_track_data_main = visualizer.generate_interactive_map_folium(
            ...
            observer_location=observer_topos_obj, # Use the created Topos object
            track_reference_time_utc=current_time_utc, 
            display_time_utc=current_time_utc, 
            track_duration_hours=app_state.get("track_duration_hours", 3.0), # Use from state or default
            min_elevation_for_footprint_deg=min_elevation_val,
            map_tiles=selected_map_style,
            cached_track_data=app_state.get('cached_track_data'), # Use cache if available
            perform_fit_bounds=True, # Fit bounds on initial load
            center_location=app_state.get("current_folium_center"), # Use calculated initial center
            current_zoom_level=app_state.get("current_folium_zoom") # Use initial zoom
        )
        ...
        app_state["folium_map_tiles"] = selected_map_style # Store selected map style
        app_state["min_elevation_for_footprint_deg"] = min_elevation_val # Store min elevation

        # Update slider visibility and range
        # Slider range: -X hours to +X hours around the track_reference_time_utc
        # track_duration_hours is for the *length* of the track from reference time
        # Let's make slider go from 0 to track_duration_hours * 60 minutes
        slider_max_minutes = int(app_state.get("track_duration_hours", 3.0) * 60)
        slider_update = gr.Slider(
            minimum=0, 
            maximum=slider_max_minutes, 
            value=0,  # Start slider at the reference time
        ...
        # Update download buttons to be visible if we have pass prediction data
        if passes:
            # Download buttons removed
            app_state['passes_data'] = passes  # Store passes data for download
            # status_log += "過境預報資料可供下載。<br>"
        else:
            # Download buttons removed
        ...
        # Ensure all 9 output values are returned even in case of a general exception
        # Initialize with default/empty values consistent with their types
        ...
        # app_state should be the last known state or an empty dict
    ...
    # Ensure a tuple of 9 items is always returned at the end of the function too.
    # This is the normal successful return path.
    return (current_pos_str_val, passes_str_val, status_log, folium_map_html_output, 
            slider_update, app_state)

# --- Function to handle GIF generation ---
def handle_gif_generation(current_app_state, gif_duration_mins, gif_fps, gif_sim_step_secs, gif_map_style, min_elevation_gif):
    ...
    try:
        ...
        # Ensure track_ref_time_utc_from_state is converted to datetime object
        ...
        # Define a unique path for the GIF to avoid conflicts if multiple users or sessions
        # For simplicity, using a timestamped filename in the data directory
        ...
        gif_output_path = visualizer.generate_footprint_animation_gif(
            ...
            # base_track_duration_hours can be taken from app_state or a default
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                # gr.Markdown("### 靜態軌跡地圖") # Removed
                # static_map_output = gr.Image(label="靜態軌跡圖", type="pil", interactive=False) # Removed
        ...
        time_slider.change(
            fn=update_map_for_time_slider, # Assuming this function is defined
        ...
        with gr.Accordion("GIF 動畫產生器", open=False):
            ...
            with gr.Row():        
                with gr.Column(scale=2): # Make image wider
                    gif_output_image = gr.Image(label="GIF 動畫", type="filepath", interactive=False, height=400) # Adjust height as needed
                    gif_status_log_output = gr.Markdown()
            
            generate_gif_button.click(
                fn=handle_gif_generation, # Assuming this function is defined
                inputs=[
                    app_state, 
                    gif_duration_input,
                    gif_fps_input,
