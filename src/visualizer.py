...
# 地球平均半徑 (公里) - 用於 footprint 計算
R_EARTH_KM_MEAN = 6371.0

# Define available map tile options
ALL_TILE_OPTIONS = {
    "OpenStreetMap": {
...
# --- New Folium Interactive Map Function ---
def generate_interactive_map_folium(satellite, 
    ...
    """
    Generates an interactive map using Folium to display satellite track,
    observer location, current satellite position (at display_time_utc), and footprint.
    Allows layer control for different map tiles.
    Can use cached_track_data to avoid recomputing the full track.
    ...
    try:
        ...
        if cached_track_data:
            ...
            # track_ref_time_from_cache_iso = cached_track_data.get("track_reference_time_utc_iso")
            # track_duration_from_cache = cached_track_data.get("track_duration_hours")
            # points_per_hr_from_cache = cached_track_data.get("points_per_hour")

            # Basic validation (optional, could be more thorough)
            # if not all([track_coordinates, track_times_iso, track_altitudes_km,
            #             track_ref_time_from_cache_iso == track_reference_time_utc.isoformat(),
            #             track_duration_from_cache == track_duration_hours,
            #             points_per_hr_from_cache == points_per_hour]):
            #     logger.warning("Cached track data mismatch or incomplete. Recomputing.")
            #     cached_track_data = None # Force recomputation
            # else:
            #     # Need to reconstruct track_end_time_utc for the end marker if relying on cache
            #     # This is already available from track_reference_time_utc + track_duration_hours
            #     pass
            if not all([track_coordinates, track_times_iso, track_altitudes_km]):
        ...
        if not cached_track_data:
            ...
            current_track_end_dt = current_track_end_skyfield_time_obj.utc_datetime() # Convert to datetime
        ...
        actual_track_end_time_utc = track_reference_time_utc + datetime.timedelta(hours=track_duration_hours) # Skyfield Time + timedelta = Skyfield Time


        # 2. Get satellite position at display_time_utc (always computed)
        ...
        if track_coordinates:
            ...
            # Intermittent track markers
            # Ensure track_times_iso and track_altitudes_km are available and have the same length as track_coordinates
        ...
        # 7. Add satellite position marker at display_time_utc
        if current_sat_lat is not None and current_sat_lon is not None:
            ...
            # 8. Calculate and add footprint at display_time_utc (always added)
        ...
        # 9. Fit bounds if requested and track_coordinates exist
...
def _plot_single_frame_for_gif(satellite, observer_location, 
                               ...
                               fig_size=(10, 8)):
    """
    Helper function to plot a single frame for the GIF.
    This will plot the full ground track and the footprint at current_time_utc.
    Returns a PIL Image.
    """
    try:
        # 1. 設定完整軌跡的時間範圍
...
def generate_footprint_animation_gif(
    ...
    """
    Generates a GIF animation of the satellite's footprint over time.

    Args:
        satellite (EarthSatellite): Skyfield satellite object.
        observer_location (Topos): Skyfield observer location object.
        animation_start_time_utc (datetime.datetime): Start time of the animation.
        animation_duration_minutes (int): Total duration of the animation in minutes.
        simulation_time_step_seconds (int): The duration in seconds that each frame of the
                                           animation represents in simulation time. Lower values
                                           mean more frames and smoother animation, but longer
                                           generation time.
        gif_playback_fps (int): Number of frames per second for the output GIF playback.
        output_gif_path (str): Path to save the generated GIF file.
        base_track_duration_hours (int): Duration of the full ground track to display in background.
        min_elevation_for_footprint_deg (float): Minimum elevation for footprint calculation.
        map_style (str): Map style for the frames.
        fig_size_inches (tuple): Figure size in inches for each frame.

    Returns:
        str | None: Path to the generated GIF file, or None if generation failed.
    """
    frames = []
...
def plot_satellite_track(satellite, observer_location, 
                         ...
                         min_elevation_for_footprint_deg=0.0,
                         map_style='Default',
                         footprint_times_utc: list[datetime.datetime] | None = None): # <-- 新增 footprint_times_utc
    """
    繪製衛星在指定時間段內的星下點軌跡以及觀測者位置。
    Args:
        satellite (EarthSatellite): Skyfield 衛星物件。
        observer_location (Topos): Skyfield 觀測者位置物件。
        start_time_utc (datetime.datetime): 軌跡開始的 UTC 時間。
        duration_hours (float): 軌跡持續時間 (小時)。
        points_per_hour (int): 每小時計算的軌跡點數量。
        min_elevation_for_footprint_deg (float): 計算 footprint 時的最小仰角。
        map_style (str): 地圖樣式 ('Default', 'Stamen Terrain', 'OpenStreetMap').
        footprint_times_utc (list[datetime.datetime] | None): 用於繪製多個 footprint 的時間點列表。
                                                            若為 None，則只繪製 start_time_utc 的 footprint。

    Returns:
        tuple[PIL.Image | None, str]: 包含繪製軌跡的圖像物件和一條關於 footprint 繪製狀態的訊息。
                                     如果繪圖失敗則圖像物件為 None。
    """
